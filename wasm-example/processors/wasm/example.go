...
//
//export alloc
func alloc(size uint64) *byte {
	buf := make([]byte, size)
	return &buf[0]
}

//export dealloc
func dealloc(ptr *byte, size uint64) {
...
func initProcessor(configPtr *byte, configLen uint64) uint32 {
...
//
//export process
func process(eventPtr *byte, eventLen uint64) uint32 {
	...
	if config != nil && config["example_value"] != nil {
...
func ptrToBytes(ptr *byte, length uint64) []byte {
	return (*[1 << 30]byte)(unsafe.Pointer(ptr))[:length:length]
}

// Helper function to convert Go byte slice to WebAssembly memory pointer
func bytesToPtr(data []byte) (*byte, uint64) {
	header := (*reflect.SliceHeader)(unsafe.Pointer(&data))
	return (*byte)(unsafe.Pointer(header.Data)), uint64(len(data))
}
